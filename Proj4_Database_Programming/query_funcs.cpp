...
void query2(connection *C, string team_color)
{
  work W(*C);
  stringstream sql;
  sql << "SELECT TEAM.NAME FROM TEAM, COLOR WHERE TEAM.COLOR_ID = COLOR.COLOR_ID AND COLOR.NAME = " << W.quote(team_color) << ";";
  W.commit();
  nontransaction N(*C);
  result R(N.exec(sql.str()));
  cout << "NAME" << endl;
...
void query3(connection *C, string team_name)
{
  work W(*C);
  stringstream sql;
  sql << "SELECT FIRST_NAME, LAST_NAME FROM PLAYER, TEAM WHERE PLAYER.TEAM_ID = TEAM.TEAM_ID AND TEAM.NAME = " << W.quote(team_name) << " ORDER BY PPG DESC;";
  W.commit();
  nontransaction N(*C);
  result R(N.exec(sql.str()));
...
void query4(connection *C, string team_state, string team_color)
{
  work W(*C);
  stringstream sql;
  sql << "SELECT FIRST_NAME, LAST_NAME, UNIFORM_NUM FROM PLAYER, STATE, COLOR, TEAM WHERE ";
  sql << "PLAYER.TEAM_ID = TEAM.TEAM_ID AND TEAM.COLOR_ID = COLOR.COLOR_ID AND TEAM.STATE_ID = STATE.STATE_ID AND ";
  sql << "STATE.NAME = " << W.quote(team_state) << " AND " << "COLOR.NAME = " << W.quote(team_color) << ";";
  W.commit();
  nontransaction N(*C);
  result R(N.exec(sql.str()));
